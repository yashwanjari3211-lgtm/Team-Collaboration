    ...
    <Draggable draggableId={task.id.toString()} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={`bg-white dark:bg-surface-900 rounded-xl shadow-sm border border-surface-200 dark:border-surface-800 group transition-all hover:shadow-md hover:border-brand-300 dark:hover:border-brand-500/50 overflow-hidden ${
            ...
            {task.labels && (
              <div className="flex flex-wrap gap-1 mb-2">
                {task.labels.split(',').map(l => (
                  <span key={l} className="px-1.5 py-0.5 text-[9px] font-bold uppercase rounded bg-brand-50 text-brand-600 dark:bg-brand-500/20 dark:text-brand-400">
                    {l}
                  </span>
                ))}
              </div>

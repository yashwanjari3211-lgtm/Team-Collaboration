...
export default function EmptyState({ type = 'messages', channelName = '', className = '' }) {
  ...
  return (
    <div className={`flex flex-col items-center justify-center py-16 px-6 text-center ${className}`}>
      ...
      <h3 className="text-base font-semibold text-surface-600 dark:text-surface-300 mb-1">{title}</h3>
      <p className="text-sm text-surface-400 dark:text-surface-500 max-w-[260px]">{subtitle}</p>
    </div>
  )
}

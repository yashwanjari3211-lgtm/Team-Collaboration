...
export default function TaskQuickAdd() {
  ...
  return (
    <form onSubmit={handleSubmit} className="px-3 py-3 border-b border-surface-700/50 flex-shrink-0">
      {convertingMessage && (
        <div className="flex items-center gap-2 mb-2 px-2 py-1.5 bg-brand-50 bg-brand-950/30 rounded-lg text-[11px] text-brand-600 dark:text-brand-400">
      ...
      <div className="flex items-center gap-2">
        <div className="flex-1 flex items-center gap-2 bg-surface-900 border border-surface-700 rounded-lg px-3 py-2 focus-within:border-brand-500/50 focus-within:ring-2 focus-within:ring-brand-500/20 transition-all">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-[13px] text-surface-800 text-studio-100 placeholder:text-surface-400 focus:outline-none"
          />
        </div>
        <button
          type="submit"
          ...
          className={`px-3 py-2 rounded-lg text-[12px] font-semibold transition-all ${
            displayTitle.trim()
              ...
              : 'bg-surface-200 bg-surface-800 text-surface-400 cursor-not-allowed'
          }`}
        >
          Add
        </button>
